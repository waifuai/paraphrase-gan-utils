...
def create_transformer_model(
    ...
    for _ in range(n_encoder_layers):
        encoder_layers.extend([tl.Relu(), tl.Dense(d_model), tl.LayerNorm()])
    ...
    for _ in range(n_decoder_layers):
        decoder_layers.extend([tl.Relu(), tl.Dense(d_model), tl.LayerNorm()])
    ...
    model = tl.Serial(
        tl.Select([0, 0]), tl.Parallel(encoder, decoder), tl.Add(), tl.LogSoftmax()
    )
    return model
